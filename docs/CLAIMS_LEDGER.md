...
|---|---|---|---|---|---|
| A-1 | "0 unsafe in production code" | SECURITY.md §In scope | `grep -rn unsafe src/` → 0 hits as of 2026-05-24 | **verified, not enforced** | A future commit could introduce `unsafe` and the claim would silently become wrong. Fix: add `#![forbid(unsafe_code)]` to `src/lib.rs`. |
...
|---|---|---|
| H-1 | A-1, A-5 | Add `#![forbid(unsafe_code)]` to `src/lib.rs`. Compile-time enforcement of the no-unsafe claim. |
...
| H-4 | A-13 | Add `examples/hmac_wrapping.rs` demonstrating the pattern for using `ag_id` with sensitive low-entropy inputs. |
